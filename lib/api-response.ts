 ...
 *  -프로덕션: 고정 메시지 반환
 */
export function responseBadRequest(message: string = 'Bad Request') {
  const isProduction = process.env.NODE_ENV === 'production'
  const clientMessage = isProduction ? PRODUCTION_ERROR_MESSAGE : message
  if (isProduction && message) {
    console.error('[API 400]', message)
  }
  return NextResponse.json({ success: false, message: clientMessage }, { status: 400 })
}

...
export class LeaderboardRepository {
  ...
  async findById(id: number): Promise<Leaderboard | null> {
    const results = await query<Leaderboard>(`SELECT * FROM ${this.tableName} WHERE id = ?`, [id])
  ...
  async getRankings(limit?: number): Promise<LeaderboardRanking[]> {
    ...
    return await query<LeaderboardRanking>(`SELECT * FROM V_리더보드_전체 ${limitClause}`)
  }
  ...
  async create(data: CreateLeaderboardDto): Promise<Leaderboard> {
    const 기록일 = data.기록일
      ? data.기록일 instanceof Date
        ? data.기록일.toISOString().split('T')[0]
        : data.기록일
